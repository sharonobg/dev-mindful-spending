 ...
 console.log('this plan id',`${spendingplan?._id}`)
  ...
  // console.log('spendingplannew',spendingplannew)
   ...
   // const planmonth = {spendingplan?.planmonth}
    ...
    const { fields, append, prepend, remove, swap, move, insert } = useFieldArray<SpendingplanType,'mycategories',
                    'mycategoriesId'>({
                      control, // control props comes from useForm (optional: if you are using FormProvider)
                      name: "mycategories",
                      keyName:"mycategoriesId" // unique name for your Field Array
                  });
   ...
   <form className="flex flex-col place-items-center" id="editform" onSubmit={handleSubmit(handleUpdateSpendingplanSubmit)}>
    ...
    {/* {getFieldState("planmonthyear").isDirty &&  */}
      ...
      <ul className="flex flex-col border-none p-0 m-0">
       {spendingplan && fields.map((field, index:number) => (
          <>
            <li key={field.mycategoriesId} className="flex flex-col">
            <label className="">Category:{field.mycategoryId +`-${index}`}/{field?.mycategoryId}</label>
            <select
            key={`mycategories.${index}.mycategoryId`}
            defaultValue={field?.mycategoryId}
            ...
            <label>Planned Amount:{field?.planamount.$numberDecimal}</label>
            <input 
            defaultValue={field?.planamount.$numberDecimal}
              id={field.planamount}
