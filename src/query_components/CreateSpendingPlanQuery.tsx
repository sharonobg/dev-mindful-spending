...
import {useCategories, useSpendingplans} from '@/query_services/queries';
...
// const inListArr = spendingplansDataNew?.data?.map((exists) => 
// `react-datepicker__month-` + (exists?.planmonth) 
// );   
const inList = spendingplansDataNew && spendingplansDataNew?.data?.map((exists) => 
 exists && new Date(` ${exists?.planyear}-${exists?.planmonth}-01`)
);
const months = [
  "Jan",
  "Feb",
...
//const availableMonths = (months, val) => arr.indexOf(val) === -1;
//const newMonths = months.includes
//
//  const notInList = spendingplans && spendingplans?.map((exists) => 
//   //exists && new Date(` ${exists?.planyear}-${exists?.planmonth}-01`)
// exists && !inList?.includes(new Date(` ${exists?.planyear}-${exists?.planmonth}-01`))
//  );
 //const included = !inList?.includes(new Date(` ${exists?.planyear}-${exists?.planmonth}-01`));
//console.log('notInList',notInList)
const inListArr = spendingplansDataNew?.data?.map((exists) => 
  //`react-datepicker__month-` + (exists?.planmonth) 
new Date(` ${exists?.planyear}-${exists?.planmonth}-01`)
); 
console.log('inListArr',inListArr)
//const availableMonths = (inList, val) => arr.indexOf(val) === -1;


  //NOTE:
  // 1. STILL NEED TITLE FOR CREATE PLAN FORM SIDE
  // 2. STILL NEED FADE ONCE ADDED, CHANGE/DISABLE BUTTON ON CLICK/ADDED- SEE VIDEO CHECK
  // 3. BUTTON TO CREATE PLAN SHOULD USE DISABLING ALSO
  // 4. POPULATE THIS FORM WITH DATA FROM ANOTHER PLAN, THEN CREATE AS USUAL
...
// }
    return(
        ...
        <div className="flex flex-col self-center place-items-center border-l-orange-100 bg-white">
        ...
        {categoriesManyQuery?.data?.length > -1 ? (categoriesManyQuery?.data?.slice(1).map((category:any,index:number ) =>
