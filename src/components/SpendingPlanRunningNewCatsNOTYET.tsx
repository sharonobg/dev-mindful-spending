...
const comboplansNewCatsNOTYET = await Transaction.aggregate([
        { $match: {
             $expr : { $eq: [ '$authorId' , { $toObjectId: userid } ] } 
        }},
  ...
  },
  {
    $project: {
      ...
      //inplanamount: "$planamount",
      planamount: { $ifNull: [ "$mycategories.mycategories.planamount", "add to plan" ] },
  ...
  },
  {
    ...
    return(
      <>
         <pre>comboplansNewCatsNOTYET:{JSON.stringify(comboplansNewCatsNOTYET, null, 2)}</pre>
      <h1>comboplansNewCatsNOTYET: `{props.fmonth}/{props.fyear}.<br /> Showing: {props.category}</h1>
    ...
    {comboplansNewCatsNOTYET?.length > 0 ? (comboplansNewCatsNOTYET.map((comboplan,index:number) =>
      <div key={index} className="horizGrid grid-cols-8 md:grid-cols-8 w-full">
            ...
            <div className="col-span-2">{comboplan?._id.planamount ? parseFloat(comboplan?._id.planamount).toFixed(2) : "not in plan"}</div>
